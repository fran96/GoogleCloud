...
namespace PubSubConsumer
{
    public class PubSubRepository
    {
        ...
        public string PullMessage(string topicName, string subscriptionName)
        {
            ...
            var response = client.Pull(GetOrCreateSubscription(subscriptionName, topicName).SubscriptionName, true, 1);

            if (response.ReceivedMessages.Count > 0)
            {
                var message = response.ReceivedMessages.ElementAt(0).Message.Data.ToStringUtf8();

                //acknowledge msg
                client.Acknowledge(GetOrCreateSubscription(subscriptionName, topicName).SubscriptionName, response.ReceivedMessages.Select(x => x.AckId));
                return message;
